import React from 'react';

function Contacto(){
  return (
    <>
      <div className='flex flex-col h-full w-full font-rubik text-white'>
        <div className='w-full h-[25%] flex items-center pl-20 font-semibold text-[5.3em]'>
          Contacta con nosotros
        </div>
        <div className='bg-[#030F3C] bg-opacity-70 h-[75%] rounded-xl m-20 mt-0 font-medium text-[2em]'>
          <div className='h-[33%] w-full flex items-center justify-between px-10 text-white'>
            <div className='h-full w-[10%] flex items-center'>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="black" className="">
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 12a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0Zm0 0c0 1.657 1.007 3 2.25 3S21 13.657 21 12a9 9 0 1 0-2.636 6.364M16.5 12V8.25" />
              </svg>
            </div>
            <div className='w-[90%] h-full flex items-center justify-center'>
              <div className=''>
                oficina@trackhub.com
              </div>
            </div>
          </div>
          <div className='h-[33%] w-full flex items-center justify-between px-10 text-white'>
            <div className='h-full w-[10%] flex items-center'>
              <svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24" strokeWidth={1.5} stroke="black" className="">
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z" />
              </svg>
            </div>
            <div className='w-[90%] h-full flex items-center justify-center'>
              <div className=''>
              976 300 804
              </div>
              </div>
            </div>
          <div className='h-[33%] w-full flex items-center justify-between px-10 text-white'>
            <div className='h-full w-[10%] flex items-center'>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="black" className="">
                <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
              </svg>
            </div>
            <div className='w-[90%] h-full flex items-center justify-center'>
            <div className=''>
            C. de Jarque de Moncayo, 10, 50012 Zaragoza
            </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Contacto;
